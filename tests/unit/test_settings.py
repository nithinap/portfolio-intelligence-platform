...
def test_settings_load_defaults():
    ...
    assert settings.sentiment_provider in {"lexicon", "openai"}
    assert settings.sentiment_openai_model
    assert settings.sentiment_openai_timeout_seconds > 0
...
def test_env_overrides_yaml(monkeypatch):
    ...
    monkeypatch.setenv("SENTIMENT_PROVIDER", "openai")
    get_settings.cache_clear()
    settings = get_settings()
    ...
    assert settings.sentiment_provider == "openai"
